...
H = M/4
fsTrian = 44100.0
xTrain = np.copy(audio)
mxTrain, pxTrain = sigCompMagSpec.compMagSpect(xTrain, window, N, M, H, fsTrian)    # Computes Mag. and Phase spectrograms
lowFreq = 0.0
highFreq = 5000.0
lowBin = int(np.ceil((lowFreq * N / float(fsTrian))))                                # Low bin index 
highBin = int(np.ceil((highFreq * N / float(fsTrian))))                              # High bin index
spectSegTime = (H * np.arange(0, np.shape(mxTrain)[0])/float(fsTrian))
binFreq = np.arange(lowBin, highBin)*float(fsTrian)/N
logmedFiltmx = np.transpose(mxTrain)

# ---------------------------------------------------------------------------------------------------------------
# Plotting the time domain waveform + spectrogram + onsets
# ----------------------------------------------------------------------------------------------------------------

plt.figure()
plt.subplot(311)
plt.plot(timeSignal, audio)
#markerline, stemlines, baseline  = plt.stem(onsetTime, tempOnsets, 'r')
#plt.setp(stemlines, 'linewidth', 2.0)
plt.xlim([0,xTrain.size/float(fsTrian)])
plt.title('(a)')  

plt.subplot(312)
plt.pcolormesh(spectSegTime, binFreq, logmedFiltmx[lowBin:highBin, :])
plt.axis([0,xTrain.size/float(fsTrian),min(xTrain),max(xTrain)])
plt.autoscale(tight=True)
plt.ylabel('Frequency(Hz)')
plt.title('(b)')  

plt.subplot(313)
plt.plot(timeEnergyCont, medfiltDetectFunc/np.max(medfiltDetectFunc), 'g', lw=2.0)
#markerline, stemlines, baseline = plt.stem(onsetTime, tempOnsets, 'r')
#plt.setp(stemlines, 'linewidth', 2.0)
plt.xlim([0,xTrain.size/float(fsTrian)])
plt.xlabel('Time(sec)')
plt.title('(c)')  
plt.savefig('SARGAM_notes_spect_detection_function.jpg')

# ----------------------------------------------------------------------------------------------------------------
# SARGAM + spectrogram + overlaid onsets
# ----------------------------------------------------------------------------------------------------------------
...
plt.xlabel('Time(sec)')
plt.title('(c)')  
plt.savefig('SARGAM_notes_spect_detected_notes.jpg')
...
plt.subplot(312)
plt.plot(f0Time, f0)
markerline, stemlines, baseline  = plt.stem(onsetTime, tempOnsets+500, 'r')
plt.setp(stemlines, 'linewidth', 2.0)
plt.xlim([0,xTrain.size/float(fsTrian)])
plt.ylabel('Frequency(Hz)')
plt.title('(b)')
...
plt.subplot(313)
plt.plot(f0Time, f0)
trueOnsetTime = (trueOnsets* hopSize)/sampleRate
markerline, stemlines, baseline = plt.stem(trueOnsetTime, np.ones(trueOnsets.size)+500, 'r')
plt.setp(stemlines, 'linewidth', 2.0)
plt.xlim([0,xTrain.size/float(fsTrian)])
plt.xlabel('Time(sec)')
plt.ylabel('Frequency(Hz)')
plt.title('(c)')
plt.savefig('SRGAM_note_pithc_correct_onstes.jpg')

# ----------------------------------------------------------------------------------------------------
# SARGAM Note Deviation detection
# ----------------------------------------------------------------------------------------------------
plt.figure()
plt.subplot(211)
plt.plot(f0Time, f0)
...
if (numberOfTrueOnsets == 8):                                                   # Checing is the number of notes are exactly 8
    for i in range(numberOfTrueOnsets):
        ...
        tempNoteTime = (np.arange(beginFrames, endFrames) * hopSize)/sampleRate
        #plt.figure()
        #plt.plot(f0Cent[beginFrames:endFrames])
        plt.plot(tempNoteTime, f0[beginFrames:endFrames], linewidth=3.0)
...
plt.savefig('SARGAM_deviation.jpg')
...
noteCorrectionRatio = idealNoteValuesHz[indxOutTuneNote]/noteValuesHz[indxOutTuneNote]      # Ratio of desired by the obtained        
...
# ----------------------------------------------------------------------------------------------------
# Original SARGAM sung by the learner
# ----------------------------------------------------------------------------------------------------
...
plt.subplot(212)
plt.pcolormesh(spectSegTime, binFreq, logmedFiltmx[lowBin:highBin, :])
plt.plot(f0Time, f0, lw=2)
plt.axis([0,xTrain.size/float(fsTrian), -1, 1])
plt.autoscale(tight=True)
plt.ylabel('Frequency(Hz)')
plt.xlabel('Time(sec)')
plt.title('(b)')  
plt.savefig('original_SARGAM_without_correction.jpg')

# --------------------------------------------------------------------------------------------------
# Corrected SARGAM sequence
# --------------------------------------------------------------------------------------------------

correctSignalTime = np.arange(correctedSignal.size)/fsTrian
mxCorrect, pxCorrect = sigCompMagSpec.compMagSpect(correctedSignal, window, N, M, H, fsTrian)    # Computes Mag. and Phase spectrograms
lowFreq = 0.0
highFreq = 600.0
lowBin = int(np.ceil((lowFreq * N / float(fsTrian))))                                # Low bin index 
highBin = int(np.ceil((highFreq * N / float(fsTrian))))                              # High bin index
spectSegTimeCorrect = (H * np.arange(0, np.shape(mxCorrect)[0])/float(fsTrian))
binFreq = np.arange(lowBin, highBin)*float(fsTrian)/N
logmedFiltmxCorrect = np.transpose(mxCorrect)

f0Corrected = np.array([])                                                   # Vector to store f0
for frame in FrameGenerator(correctedSignal, frameSize=frameSize, hopSize=hopSize):
    frame = run_windowing(frame)
    spectrum = run_spectrum(frame)
#    detectionFunction = np.append(detectionFunction, onsetDetection(spectrum[50:200], spectrum[50:200]))
#    energy = np.append(energy, timeEnergy(frame))
    f0t = pitchYin(spectrum)                                        # YIN outputs pitch and its confidence 
    f0Corrected = np.append(f0Corrected, f0t[0])


plt.figure()
plt.subplot(211)
plt.plot(correctSignalTime, correctedSignal)
#markerline, stemlines, baseline  = plt.stem(onsetTime, tempOnsets, 'r')
#plt.setp(stemlines, 'linewidth', 2.0)
plt.xlim([0,correctedSignal.size/float(fsTrian)])
plt.title('(a)')  

f0TimeCorrect = (np.arange(f0Corrected.size) * hopSize)/sampleRate
plt.subplot(212)
plt.pcolormesh(spectSegTimeCorrect, binFreq, logmedFiltmxCorrect[lowBin:highBin, :])
plt.plot(f0Time, f0, lw=2)
plt.plot(f0TimeCorrect, f0Corrected, 'g')
plt.axis([0,correctedSignal.size/float(fsTrian), -1, 1])
plt.autoscale(tight=True)
plt.ylabel('Frequency(Hz)')
plt.title('(b)')  
plt.xlabel('Time(sec)')
plt.savefig('SARGAM_corrected.jpg')

 
import waveio as io
io.wavwrite(correctedSignal, sampleRate, 'traineeMelody.wav')
