...
#	Ns = 512
    ...
    #fs = np.load('Tanfs.npy')
    #x = np.load('TanSig.npy')
...
def transformation_synthesis(freqScaling = np.array([0, 1.5, 1, 1.5])):
	...
	tfreq = np.load('tfreq.npy')
	tmag = np.load('tmag.npy')
	fs = 44100                     # TODO: Tanpura sampling frequency is hard coded
	# size of fft used in synthesis
	Ns = 512
...
def mainTanpuraSynth(freqScal):
	...
	transformation_synthesis (freqScaling=freqScal)
