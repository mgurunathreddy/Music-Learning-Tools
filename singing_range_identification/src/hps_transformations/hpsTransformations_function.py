...
def analysis(inputFile='tanpura.wav', window='blackman', M=601, N=1024, t=-100, 
	minSineDur=0.1, nH=100, minf0=50, maxf0=1000, f0et=5, harmDevSlope=0.01, stocf=0.1):
	"""
...
def transformation_synthesis(inputFile, fs, hfreq, hmag, mYst, freqScaling = np.array([0, 1, 2.01, 1, 2.679, 1, 3.146, 1]), 
	...
	hfreqt, hmagt = HT.harmonicFreqScaling(hfreq, hmag, freqScaling, timbrePreservation, fs)
...
#	y, yh, yst = HPS.hpsModelSynth(yhfreq, yhmag, np.array([]), ystocEnv, Ns, H, fs)
	...
	# synthesis from the trasformed hps representation 
	y, yh, yst = HPS.hpsModelSynth(hfreqt, hmagt, np.array([]), mYst, Ns, H, fs)

	# write output sound 
	outputFile = 'hpsModelTransformation.wav'
	UF.wavwrite(y,fs, outputFile)
