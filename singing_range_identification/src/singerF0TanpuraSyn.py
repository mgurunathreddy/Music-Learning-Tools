...
normZFFS = zffs/np.max(np.abs(zffs))
sqrdNormZFFS = np.power(normZFFS, 2)
...
runningAvgNormZFFS = movingaverage(sqrdNormZFFS, 400)
thresh = 1 - np.exp(-10 * runningAvgNormZFFS) 
threshSamples = np.where(thresh > 0.3)[0]
voicBeg = threshSamples[0]
voicEnd = threshSamples[-1]
...
tanpuraTuningFreq = 139                         # Our Tanpura is tuned to 139Hz (get it from spectrogram)
scalingFactor = medianF0/tanpuraTuningFreq
